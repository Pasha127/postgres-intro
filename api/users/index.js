...
userRouter.get("/", async (req,res,next)=>{
    try{
        ...
        if (req.query.lastName) query.lastName = {[Op.iLike]:`%${req.query.lastName}%`}
        if (req.query.country) query.country = {[Op.iLike]:`%${req.query.country}%`}
        if (req.query.email) query.email = {[Op.iLike]:`%${req.query.email}%`}
        if (req.query.age) query.age = {[Op.between]:req.query.age.split(",")}
        if (req.query.username) query.username = {[Op.iLike]:`%${req.query.username}%`}
...
userRouter.get("/:userId" , async (req,res,next)=>{
    try{
        ...
        const foundUser = await UserModel.findByPk(req.params.productId, {
            attributes: req.query.attributes? req.query.attributes.split(","):{}})     
        if(foundUser){
